          ...
          async.eachSeries(data,function(item,callback){
              db.query(`SELECT * FROM column_table WHERE \
                ...
                if(err){
                  ...
                  db.query(`SELECT * FROM article_table WHERE category_ID=5 \
                    ORDER BY weight DESC LIMIT 0,1`,(err,data)=>{
                     if(err){
                        console.log("find 理事会介绍 err："+err);
                        res.status(500).send("db err").end();
                     }else{
                        data=JSON.parse(JSON.stringify(data))
                        indexData.lsh=data[0].content;
                                  ...
                                  category_ID=5 ORDER BY weight \
                                    DESC LIMIT 0,1`,(err,rightlsh_data)=>{
                                      if(err){
                                         ...
                                         res.render('web/five.html',{indexData:indexData})
                                      }
                                   })     
                                 }
                            })
